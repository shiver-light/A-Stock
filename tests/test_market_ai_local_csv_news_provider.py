...
class LocalCsvNewsProviderTestCase(unittest.TestCase):
    ...
    def test_fetch_news_skips_rows_marked_filtered(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            csv_path = Path(tmpdir) / "news.csv"
            pd.DataFrame(
                {
                    "news_id": ["n1", "n2"],
                    "source": ["cls", "cls"],
                    "title": ["某股涨超5%", "政策支持算力建设"],
                    "published_at": ["2026-09-12T10:00:00+08:00", "2026-09-12T10:30:00+08:00"],
                    "is_filtered": [True, False],
                    "filter_reason": ["quote_broadcast", ""],
                }
            ).to_csv(csv_path, index=False)
            provider = LocalCsvNewsProvider(csv_path)
        ...
        self.assertEqual([item.news_id for item in items], ["n2"])
