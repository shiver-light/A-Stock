"""A-share index daily market data."""
...
INDEX_DAILY_FIELDS = ["ts_code", "trade_date", "open", "close", "pre_close"]
...
class IndexDailyRequest:
    ...
    start_date: str
    end_date: str
    fields: tuple[str, ...] = tuple(INDEX_DAILY_FIELDS)
    refresh: bool = False


class AShareIndexDailyService:
    """Minimal cached wrapper for Tushare index_daily."""
    ...
    def get_index_daily(self, request: IndexDailyRequest) -> pd.DataFrame:
        self._validate_request(request)
        cache_path = self.cache_dir / "index_daily" / f"{request.ts_code}.parquet"
        ...
        for start_date, end_date in missing_ranges:
            fetched = self.client.index_daily(
    ...
    def _validate_request(self, request: IndexDailyRequest) -> None:
...
def get_a_share_index_daily(
    *,
    ...
    service = AShareIndexDailyService(cache_dir=cache_dir)
    request = IndexDailyRequest(
        ts_code=ts_code,
        start_date=start_date,
        end_date=end_date,
        refresh=refresh,
    )
    return service.get_index_daily(request)
